...
SELECT f.* from film as f where f.film_id
NOT IN (select i.film_id FROM inventory as i);
...
select p.customer_id, c.first_name, c.last_name, p.amount, p.payment_date, s.last_name
from payment as p
